...
開催回全体のアーカイブ情報を追加・更新する場合は、このファイルを編集してください。

### reader-archives-data.js

読み手ごとの個別朗読アーカイブ情報を管理します。

個別朗読動画のYouTube IDやサムネイルを追加・更新する場合は、このファイルを編集してください。
...
開催回全体の過去イベントは `event-archives-data.js` 内の `archives` 配列で管理します。
...
読み手別の個別朗読アーカイブは `reader-archives-data.js` 内の `readerArchives` 配列で管理します。
...
開催回タイトルや開催日は `eventId` を使って `event-archives-data.js` から取得する構造を維持してください。

個別朗読動画のYouTube IDやサムネイルを追加するだけで、読み手紹介の写真ボタンとモーダル表示へ反映される構造を維持してください。
